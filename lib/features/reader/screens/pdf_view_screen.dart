...
class _PdfViewScreenState extends State<PdfViewScreen> {
  ...
  int _cropMargins = 0; // 0=off, 10, 15, 20, 25 percent
  ...
  void initState() {
    super.initState();
    WakelockPlus.enable();
    ScreenBrightness().application.then((v) { if (mounted) setState(() => _brightness = v); });
  ...
  void _saveBookSettings() {
    ...
    _bookSettingsService!.saveSettings(widget.bookId!, BookSettings(
      ...
      lastZoom: _viewerController.currentZoom,
    ));
  }
  ...
  void _restoreZoom() {
    ...
    if (z > 1.0) Future.delayed(const Duration(milliseconds: 300), () { if (mounted) _viewerController.setZoom(_viewerController.centerPosition, z); });
  }
  ...
  void _showGoToPageDialog() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(controller: ctrl, keyboardType: TextInputType.number, autofocus: true, decoration: InputDecoration(hintText: '1 - $_totalPages')),
        actions: [
  ...
  @override
  void dispose() {
    _saveDebounce?.cancel(); _readingTimer?.cancel(); _searchCtrl.dispose(); _textSearcher?.dispose(); _ttsService.removeListener(_onTtsStateChanged); if (!_closed) _saveProgress();
    ...
    return PopScope(canPop: false, onPopInvokedWithResult: (didPop, _) { if (!didPop) { if (_isSearching) { setState(() => _isSearching = false); _searchCtrl.clear(); _textSearcher?.resetTextSearch(); } else { _closeAndPop(); } } },
      child: Focus(
        ...
        onKeyEvent: (node, event) {
          if (event is KeyDownEvent && !_textViewController.textViewMode && !_isSearching) {
            if (event.logicalKey == LogicalKeyboardKey.audioVolumeUp) {
              if (_currentPage + 1 < _totalPages) _viewerController.goToPage(pageNumber: _currentPage + 2);
              return KeyEventResult.handled;
            }
            if (event.logicalKey == LogicalKeyboardKey.audioVolumeDown) {
              if (_currentPage > 0) _viewerController.goToPage(pageNumber: _currentPage);
              return KeyEventResult.handled;
            }
          }
        ...
        child: Scaffold(
        appBar: _fullscreen ? null : (_isSearching ? _uiBuilder.buildSearchBar(context: context, searchController: _searchCtrl, textSearcher: _textSearcher, onBackPressed: () { setState(() => _isSearching = false); _searchCtrl.clear(); _textSearcher?.resetTextSearch(); }, onSearchSubmitted: (q) { if (q.isNotEmpty) _textSearcher?.startTextSearch(q); }) : _uiBuilder.buildAppBar(context)),
        ...
        body: Stack(children: [
          ...
          if (_horizontalScroll && !_textViewController.textViewMode) ...[
            Positioned(left: 0, top: 0, bottom: 0, width: MediaQuery.of(context).size.width * 0.25, child: GestureDetector(behavior: HitTestBehavior.translucent, onTap: () { if (_currentPage > 0) _viewerController.goToPage(pageNumber: _currentPage, duration: const Duration(milliseconds: 300)); })),
            Positioned(right: 0, top: 0, bottom: 0, width: MediaQuery.of(context).size.width * 0.25, child: GestureDetector(behavior: HitTestBehavior.translucent, onTap: () { if (_currentPage + 1 < _totalPages) _viewerController.goToPage(pageNumber: _currentPage + 2, duration: const Duration(milliseconds: 300)); })),
          ...
          if (!_fullscreen && _totalPages > 1 && !_isSearching) Positioned(bottom: 0, left: 16, right: 16, child: SliderTheme(data: SliderTheme.of(context).copyWith(showValueIndicator: ShowValueIndicator.onDrag), child: Slider(value: _currentPage.toDouble(), min: 0, max: (_totalPages - 1).toDouble(), divisions: _totalPages - 1, label: 'Page ${_currentPage + 1}', onChanged: (v) { final p = v.round(); _viewerController.goToPage(pageNumber: p + 1); }))),
          ...
          if (_totalPages > 0 && _fullscreen) Positioned(bottom: 0, left: 0, right: 0, child: LinearProgressIndicator(value: (_currentPage + 1) / _totalPages, minHeight: 2, backgroundColor: Colors.transparent)),
        ]),
      ),
      ),
    );
